...
const argv = yargs(hideBin(process.argv))
  .command('run <mode>', 'enqueue a run', (y) =>
    y
      .positional('mode', { choices: ['full', 'custom', 'single'] as const })
...
(async () => {
  const modeMap: Record<string, 'FULL' | 'CUSTOM' | 'SINGLE'> = { full: 'FULL', custom: 'CUSTOM', single: 'SINGLE' };
  const mode = modeMap[argv.mode as string];
